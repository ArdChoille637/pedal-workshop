...
public final class WorkshopStore {
    ...
    public func loadAll() async {
        ...
        do {
            ...
            let schems = try await store.fetchSchematics()
    ...
    /// Human-readable notes for suppliers that errored during the last search
    /// (e.g. "Mammoth: HTTP 429"). Empty when every supplier responded.
    /// Distinct from `error` — a supplier being down is a notice, not a failure.
    public var supplierSearchNotices: [String] = []

    /// Search all configured suppliers in parallel.
    /// Mammoth requires no key; Mouser is included only if `mouserAPIKey` is
    /// non-empty. Per-supplier failures land in `supplierSearchNotices`.
    public func searchSuppliers(query: String) async -> [SupplierSearchResult] {
        let outcome = await SupplierSearchService.shared.searchAll(query: query, mouserKey: mouserAPIKey)
        supplierSearchNotices = outcome.failures
    ...
    public func saveSupplierListing(_ result: SupplierSearchResult, for component: Component) async {
        guard let supplierId = supplierIdForSlug(result.supplierSlug) else {
            // Refuse to persist a dangling supplier_id: 0 reference.
            self.error = "Unknown supplier '\(result.supplierSlug)' — listing not saved."
            return
        }
        do {
    ...
    private func supplierIdForSlug(_ slug: String) -> Int? {
        suppliers.first(where: { $0.slug == slug })?.id
    }
}
