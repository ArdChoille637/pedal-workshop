...
public struct InventoryView: View {
    ...
    public var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .automatic) {
                ...
                .toggleStyle(.button)
                .help("Show only components below their minimum quantity")
            }
            ToolbarItem(placement: .automatic) {
                Button {
    ...
    private var filtered: [Component] {
        let base = lowStockOnly
            ? store.filteredComponents.filter(\.isLowStock)
            : store.filteredComponents
        // Stable, browsable ordering instead of raw JSON insertion order.
        return base.sorted { ($0.category, $0.value) < ($1.category, $1.value) }
    }
}
