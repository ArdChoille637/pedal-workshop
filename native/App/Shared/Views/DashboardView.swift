                    ...
                    if store.isLoading {
                        ...
                        VStack(spacing: 12) {
                            ContentUnavailableView(
                                "No Data",
                                systemImage: "tray",
                                description: Text("Click Reload to load your workshop data.")
                            )
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if items.isEmpty {
                ...
                ForEach(items) { status in
                    // A launchpad, not a poster: tier rows navigate to the project.
                    if let project = store.projects.first(where: { $0.id == status.projectId }) {
                        NavigationLink(destination: ProjectDetailView(project: project)) {
                            ProjectTierRow(project: status)
                        }
                        .buttonStyle(.plain)
                    } else {
                        ProjectTierRow(project: status)
