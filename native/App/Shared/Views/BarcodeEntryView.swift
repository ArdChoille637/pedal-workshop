...
struct BarcodeEntryView: View {
    ...
    private func process() {
        ...
        // Match by MPN, supplier part number, or value — each check is
        // independent (the old early-return on an MPN mismatch skipped the
        // remaining checks for that component and missed valid matches).
        let comp = store.components.first { c in
            if let mpn = parsed.mpn, let cMPN = c.mpn,
               mpn.lowercased() == cMPN.lowercased() { return true }
            if let spn = parsed.supplierPN, let cMPN = c.mpn,
               spn.lowercased() == cMPN.lowercased() { return true }
            if let mpn = parsed.mpn,
               c.value.lowercased() == mpn.lowercased() { return true }
            return false
        }
