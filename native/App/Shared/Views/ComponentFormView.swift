...
public struct ComponentFormView: View {
    ...
    private func save() {
        saving = true
        store.error = nil   // clear stale errors so the check below is ours
        Task {
            ...
            // Keep the sheet open and show the error instead of silently
            // discarding the user's input when the save failed.
